...
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = {"http://127.0.0.1:5500", "http://localhost:5500", "http://127.0.0.1:3000", "http://localhost:3000"}, allowedHeaders = "*", methods = {RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT, RequestMethod.DELETE, RequestMethod.OPTIONS})
public class CourseController {
    ...
    public ResponseEntity<Map<String, Object>> getCourseStats() {
        try {
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("total_courses", courses.size());
            ...
            response.put("courses_by_status", statusCounts);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return createErrorResponse("Failed to retrieve course statistics: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
